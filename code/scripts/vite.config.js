...
export default defineConfig(({ mode }) => {
  ...
  plugins: [
    ...
    }),
    {
      ...
      transformIndexHtml(html, ctx) {
        ...
        const relativePath = path.relative(resolve(__dirname, 'windows'), ctx.filename);
        // 额外增加两个层级
  ...
  build: {
    ...
    rollupOptions: {
      input: {
        main: resolve(__dirname, 'windows/index.html'), // 使用输出目录的文件
        help: resolve(__dirname, 'windows/help-window/index.html'), // 使用输出目录的文件
        settings: resolve(__dirname, 'windows/settings-window/index.html'), // 使用输出目录的文件
      },
      output: {
				dir:'./dist'
			},
